...
v2 대비 달라진 핵심 3가지
  ...
  2. vertical 통계량을 "그 데이터 자신의 캠페인"에서 뽑을 수 있게 했다.
     공격자는 공격 트레이스를 갖고 있으므로 그 평균/표준편차를 계산하는 것은
     라벨 누설이 아니며 완전히 정당하다.
     단, 실측 결과 이 데이터셋에서는 "external"(v2 방식)과 차이가 없다
     (전이 상관 0.7385 vs 0.7386). 캠페인 격차 자체가 거의 없기 때문이다.
     캠페인이 실제로 어긋난 데이터셋을 위한 안전장치로 남겨둔다.
...
def per_trace_scale(X, estimator="mad"):
    ...
    실측 결론: 이 데이터셋에서는 효과가 거의 없다.
      캠페인 내부  |rho| 0.7427 -> 0.7434
      캠페인 간    전이 상관 0.7376 -> 0.7385
    두 경우 모두 개선폭이 0.001 수준이다. DSCAD의 프로파일링/공격 캠페인이 매우
    잘 정합되어 있어(평균 이동 중앙값 0.044 sigma, 분산비 1.001) 보정할 편차가
    애초에 없다.

    측정 장비나 세션이 다른 데이터셋에서는 의미가 있을 수 있다. 비용이 거의 없고
    해롭지 않으므로 기본 활성으로 두되, **이 데이터셋에서 성능 개선을 기대하지 말 것.**
    """
    if estimator == "mad":
